...
class TestUser(TestCase):
    ...
    def test_add_in(self):
        ...
        self.assertIn(self.message, list(self.user_to.incoming_messages.values()))

    def test_add_out(self):
        self.user_to.add_message(self.message1, False)
        self.assertIn(self.message1, list(self.user_to.outgoing_messages.values()))

    def test_get_last(self):
        self.assertEqual(self.user_to.incoming_messages.popitem()[1], self.message2)
    ...
    def test_read_last(self):
        ...
        self.assertEqual(self.user_to.incoming_messages.popitem()[1], self.message)

    def test_read_all(self):
        self.assertEqual(self.user_to.read_all_messages(), [self.message, self.message2])
        self.assertEqual(bool(self.user_to.incoming_messages), False)
...
class TestMessegeHelper(TestCase):
    ...
    def test_send_message(self):
        self.assertEqual(main.UserHelper.get_user(self.user_to_id).get_last_message().text,
                         "Hello bro")
        self.assertEqual(main.UserHelper.get_user(self.user_from_id).outgoing_messages.popitem()[1].text,
                         "Hello bro")
        us = main.User("Fail")
